...
Usage:
    python -m fhirbench.harness.report --run-id dryrun-10p
    python -m fhirbench.harness.report --run-id full-100k --results-root results/loadtest
"""
...
def disk_used_bytes(disk_json_path: Path, server_id: str) -> int | None:
    ...
    from fhirbench.harness.resources import parse_size
    total = 0.0
    matched = False
    for v in vols:
        name = v.get("Name", "")
