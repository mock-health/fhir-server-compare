...
Usage:
  python -m loadtest.conformance.run \\
      --round 2026-q2-r000 \\
      --server hapi \\
      --testscripts conformance/testscripts/fhir-r4-base
  python -m loadtest.conformance.run --round 2026-q2-r000 --server all
"""
...
def _load_applicability(profile_id: str) -> dict | None:
    ...
    path = REPO_ROOT / "conformance" / "profiles" / f"{profile_id}.yaml"
    if not path.is_file():
        return None
    try:
...
def main() -> None:
    ...
    if not args.skip_smoke_check:
        unreachable = smoke_check(REPO_ROOT / "servers.yaml", targets)
        if unreachable:
            print(
    ...
    servers_cfg = load_servers(REPO_ROOT / "servers.yaml") if applicability else None
