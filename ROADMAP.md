...
- **k6 port of the load-test harness — shadow-run validation.** The k6 workloads are in the tree at [`src/fhirbench/k6/`](./src/fhirbench/k6/) (CRUD + Search, full parity with [`src/fhirbench/harness/workload_crud.py`](./src/fhirbench/harness/workload_crud.py) mix-mode and [`src/fhirbench/harness/workload_search.py`](./src/fhirbench/harness/workload_search.py)). A Python context emitter ([`src/fhirbench/cli/emit_k6_context.py`](./src/fhirbench/cli/emit_k6_context.py)) resolves `config/servers.yaml` + `config/queries.yaml` + env vars in one place so k6 never sees YAML. A post-processor ([`src/fhirbench/k6/postprocess.py`](./src/fhirbench/k6/postprocess.py)) reshapes k6's raw NDJSON into the `crud.jsonl` / `search.jsonl` the existing `cell_summary.py` + `parse_report.py` pipeline already understands, so the round artifact flow stays untouched. The shadow-run validator ([`src/fhirbench/cli/compare_harnesses.py`](./src/fhirbench/cli/compare_harnesses.py)) diffs two `benchmark.json` rounds cell-by-cell at ±10% p50 / ±0.5pp error rate / ±10% ok-throughput. Remaining work before the default runner flips from Python to k6: (1) wire `src/fhirbench/harness/ramp.py` to call `k6 run` per cell instead of the Python workloads, (2) run one shadow round across the full roster, (3) gate cutover on `compare_harnesses.py` → green. Phased CRUD mode (C/U/R/V/D) and Bulk Data async-polling are follow-ups; v0 ports only the mix-mode CRUD and the Search workload that the Python harness already publishes numbers for.
